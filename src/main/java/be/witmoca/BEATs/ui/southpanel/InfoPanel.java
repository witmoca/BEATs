...
class InfoPanel extends JPanel implements ListSelectionListener {
	 ...
	 * 
	 * @param trackingTable
	 *            A JTable that gets its selection tracked and translated into
	 *            information
	 * @param artistColumn
	 *            The column (of trackingTable) that contains a String representing
	 *            an artist
	 * @param songColumn
	 *            The column (of trackingTable) that contains a String representing
	 *            a song title
	...
	public InfoPanel(JTable trackingTable, int artistColumn, int songColumn) {
		...
		GridBagConstraints left = new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.NORTHWEST,
				GridBagConstraints.NONE, new Insets(5, 10, 0, 10), 0, 0);
		GridBagConstraints right = new GridBagConstraints(1, 0, 1, 1, 1, 0, GridBagConstraints.NORTHWEST,
	...
	public void valueChanged(ListSelectionEvent arg0) {
		...
		try {
			...
			try (PreparedStatement selLastArtist = SQLConnection.getDbConn().prepareStatement(
					"SELECT Title, SongsInArchive.EpisodeId, Max(EpisodeDate) FROM SongsInArchive, Song, Episode WHERE SongsInArchive.songId = Song.songId AND SongsInArchive.EpisodeId = Episode.EpisodeID AND ArtistName = ?")) {
				selLastArtist.setString(1, artist);
				ResultSet rs = selLastArtist.executeQuery();
				if (rs.next()) {
					artistLast.setText(rs.getInt(2) + " ("
							+ DateTimeFormatter.ofPattern("dd/MM/uu").format(LocalDate.ofEpochDay(rs.getLong(3)))
							+ ") - " + rs.getString(1));
				} else {
					return;
				}
			...
			try (PreparedStatement selLastSong = SQLConnection.getDbConn().prepareStatement(
					"SELECT SongsInArchive.EpisodeId, max(EpisodeDate), count(title) FROM SongsInArchive, Song, Episode WHERE SongsInArchive.songId = Song.songId AND SongsInArchive.EpisodeId = Episode.EpisodeID AND ArtistName = ? AND title = ?")) {
				selLastSong.setString(1, artist);
				selLastSong.setString(2, song);
				ResultSet rs = selLastSong.executeQuery();
				if (rs.next() && rs.getInt(3) != 0) {
					songLast.setText(rs.getInt(1) + " ("
							+ DateTimeFormatter.ofPattern("dd/MM/uu").format(LocalDate.ofEpochDay(rs.getLong(2)))
							+ ")");
